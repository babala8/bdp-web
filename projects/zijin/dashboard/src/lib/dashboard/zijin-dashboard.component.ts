...
@Component({
  ...
  template: `
    <!-- 提供更改主题数目的操作 -->
    <nz-dropdown nzTrigger="click" nzPlacement="bottomRight" style="margin-top: 16px">
      <i nz-dropdown nz-icon type="setting" theme="outline"></i>
...
export class ZijinDashboardComponent implements OnInit {
  ...
  configPages() {
    ...
    const subject = this.modal.create({
      nzTitle: '选择系统内置主题',
      nzWidth: 600,
      nzMaskClosable: false,
      nzContent: BuiltInAddDashboardComponent,
      nzComponentParams: {
        pages: pendingPages,
      },
    });
    subject.afterClose.subscribe(data => {
      console.log(data);
      if (data.selected) {
  ...
  openUserAdd() {
    ...
    this.modal.create({
      ...
      nzFooter: null,
      nzMaskClosable: false,
      nzContent: UserAddDashboardComponent,
      nzOnOk: () => {
        this.getMultiPagesMeta();
      },
    });
